...
export function ProviderGenerator({title,kind,description}:Props){
 const supabase=getSupabaseBrowserClient();const[projects,setProjects]=useState<Project[]>([]);const[projectId,setProjectId]=useState('');const[input,setInput]=useState('');const[style,setStyle]=useState('cinematic');const[busy,setBusy]=useState(false);const[result,setResult]=useState('');const[voices,setVoices]=useState<Array<{voice_id:string;name:string}>>([]);const[voice,setVoice]=useState('')
 useEffect(()=>{if(kind!=='voice')return;void fetch('/api/providers/voices').then(r=>r.json() as Promise<{voices?:Array<{voice_id:string;name:string}>}>).then(j=>setVoices(j.voices??[])).catch(()=>setVoices([]))},[kind])
