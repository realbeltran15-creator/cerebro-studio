...
export default function OpportunitiesPage(){
 ...
 return <StudioShell title="Oportunidades"><div className="hero"><div><small>DETECCIÓN → PROYECTO</small><h2>Oportunidades accionables</h2><p>Revisa evidencia observada y convierte una señal en un proyecto real.</p></div><div className="status"><b>{items.length}</b><span>oportunidades disponibles</span><span>Publicación real bloqueada</span></div></div>{error&&<p className="error">{error}</p>}{loading?<p>Cargando…</p>:<div className="grid">{items.map(o=><article key={o.id}><small>{o.region||'Global'} · {o.language||'—'}</small><h3>{o.title}</h3><p>Estado: {o.status}</p><p>Fuente: {o.source_platform}</p>{o.source_id&&/^https:\/\/(www\.|m\.)?(youtube\.com|youtu\.be)\//.test(o.source_id)&&<p><a href={o.source_id} target="_blank" rel="noopener noreferrer">Abrir fuente original ↗</a></p>}{Array.isArray(o.evidence)&&o.evidence.map((ev,i)=>{if(!ev||typeof ev!=='object'||Array.isArray(ev))return null;const note=ev['note'];return typeof note==='string'&&note.trim()?<p key={i}>Observación: {note}</p>:null})}<form onSubmit={e=>createProject(e,o)}><button disabled={Boolean(o.project_id)}>{o.project_id?'Proyecto creado':'Convertir en proyecto'}</button></form></article>)}</div>}</StudioShell>
}